...
public class RopeLogic : Projectile 
{
    ...
    protected override void Start() 
    {
        ...
        speed = ropeManager.ropeHookSpeed;
        transform.name = "RopeHook";
        Physics2D.IgnoreLayerCollision(layerPlayer, layerHook, true);
        GetComponent<Rigidbody2D>().velocity = transform.up * speed;
        GetComponent<Rigidbody2D>().drag = ropeManager.ropeHookSpeedDamp;
        ...
        // VFX 로프 렌더러 가져오기
        ropeRenderer = owner.GetComponent<RopeRenderer>();
        if (ropeRenderer == null)
        {
            Debug.LogWarning("VFXRopeRenderer를 찾을 수 없습니다. owner에 VFXRopeRenderer 컴포넌트를 추가하세요.");
        }
    }
    ...
    void FixedUpdate() 
    {
        if (!hooked)
        {
            ...
            if (Vector2.Distance(owner.transform.position, transform.position) > ropeManager.maxLength)
                ropeManager.DestroyHook();
        }
        ...
        float allowedDistance = ropeManager.maxLength - combinedAnchorLen;
        spring.distance = Mathf.Clamp(spring.distance + Input.GetAxis("Vertical") * -1 * ropeManager.ropeClimbSpeed, 1, allowedDistance);
    ...
    void RopeJointManager()
    {
        ...
        if (anchors.Count > 1)
        {
            Vector2 ABVector = new Vector2(anchors[anchors.Count - 1].x - owner.transform.position.x, 
                                          anchors[anchors.Count - 1].y - owner.transform.position.y).normalized;
    ...
    void UpdateVFXRope()
    {
        if (ropeRenderer != null)
        {
            ropeRenderer.UpdateRope(anchors, owner.transform.position, transform.position, hooked);
        }
    }
